...
class Users extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchUsers(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
    let role=permissionCheck("users");
      if(!role)
        return <Nopermission/>
             ...
             <div className="transition-item list-page">
                <div className="main-content">
                   <div className="col s12 portlet-title"> 
                      <div className="caption">USERS
                      </div>
                    </div>

                  <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-users":""}/>
                { action.indexOf("View")!==-1 &&            
                   <table className="striped">
                                <thead>
                                  <tr>
                                    <th className="leftAlignText">Name</th>
                                    <th>Business ID</th>
                                    <th className="leftAlignText">Username</th>
                                    <th className="leftAlignText">Email</th>
                                    <th>Mobile No</th>
                                    <th className="leftAlignText">Designation</th>
                                  ...
                                  data.data && data.data.map((val,i)=>{
                                    ...
                                    return(
                                      <tr key={i}>
                                          ...
                                          <td>{val.businessid}</td>
                                          <td className="leftAlignText">{val.username}</td>
                                          <td className="leftAlignText">{val.email}</td>
                                          <td>{val.mobile}</td>
                                          <td className="leftAlignText">{val.designation}</td>
                                          <td className="leftAlignText">{val.address}</td>
                                          <td>
                                            <span className={`text-white badge ${(val.status==1)?'green':'red'}`}>{(val.status==1)?'Active':'InActive'}</span>
                                          </td>
                                          <td>
                                             { action.indexOf("Edit")!==-1 &&
                                                <Link to={`settings/general/edit-users/${val.user_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                                              }&nbsp;&nbsp;
                                            {action.indexOf("Delete")!==-1 &&
                                                <span onClick={(e)=>{this.delete(val.user_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                                              }
                                          </td>
                                      </tr>
                                      )
