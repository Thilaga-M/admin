...
import { fetchRoles,deleteRoles } from '../../actions/rolesActions';
...
class Roles extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchRoles(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
    let role=permissionCheck("roles");
      if(!role)
        return <Nopermission/>
             ...
             <div className="transition-item list-page">
                <div className="main-content">
                  <div className="col s12 portlet-title"> 
                      <div className="caption">ROLES AND PERMISSIONS
                      </div>
                    </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-roles":""}/>
                { action.indexOf("View")!==-1 &&
                      <table className="striped">
                        <thead>
                          <tr>
                            <th className="leftAlignText">Role Name</th>
                            <th className="leftAlignText">Description</th> 
                            <th className="leftAlignText">Action</th>
                          </tr>
                        </thead>
                        <tbody>
                        {
                          data.data && data.data.map((val,i)=>{
                            ...
                            return(
                              <tr key={i}>
                                  <td className="leftAlignText">{val.rolename}</td>
                                  <td className="leftAlignText">{val.description}</td>
                                  <td className="leftAlignText">
                                    { action.indexOf("Edit")!==-1 &&
                                    <Link to={`settings/general/edit-roles/${val.role_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                                    }
                                    &nbsp;&nbsp;
                                    {action.indexOf("Delete")!==-1 &&
                                      <span onClick={(e)=>{this.delete(val.role_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                                    }
                                  </td>
                              </tr>
                              )
...
const RolesContainer = connect(mapStateToProps,
