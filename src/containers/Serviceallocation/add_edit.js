...
import {
  ...
  update_fetch_status,
  viewServiceallocation,
  servicemasterlist,
  viewBill,
  confirmBill
} from "../../actions/serviceallocationActions";
...
class AddEdit extends React.Component {
  ...
  eventHandle(e, key) {
    if (key === "discount") {
      let discount = e.target.value
        ? (this.state.allocate_office_price * e.target.value) / 100
        : 0;
      let discount_price = this.state.allocate_office_price - discount;
      this.setState({
        [key]: e.target.value,
        allocate_discount_price: discount_price
      });
    } else {
  ...
  onsubmit() {
    ...
    this.state.servicelist.map((val, i) => {
      if (parseInt(this.state.servicelist[i].selected["amount"]) == 0) {
        errorMsg +=
          "Amount should not be zero for " +
          this.state.servicelist[i].selected["servicename"] +
          "\n";
        errorCount++;
      } else if (
        this.state.servicelist[i].selected.startDate == "" ||
        this.state.servicelist[i].selected.endDate == ""
      ) {
        errorMsg +=
          "Start Date or End Date is missing for " +
          this.state.servicelist[i].selected["servicename"] +
          "\n";
        errorCount++;
      }
    ...
    if (confirmation == true) {
      ...
      confirmBill(localStorage.getItem("sa_hid")).then(res => {
        if (res.data.status === 200) {
          ...
          browserHistory.push("billing/bill-reporting");
        }
      });
    }
  }
  ...
  onSelectedtext(i, val, list) {
    ...
    this.state.servicelist[i].selected.amount =
      this.state.servicelist[i].selected.qty *
      this.state.servicelist[i].selected.rate;
    this.state.servicelist[i].selected.startDate =
      this.state.servicelist[i].selected["servicetype"] == 1 || allocate_id == 1
        ? today
        : "";
    this.state.servicelist[i].selected.endDate =
      this.state.servicelist[i].selected["servicetype"] == 1 || allocate_id == 1
        ? today
        : "";
    if (this.state.servicelist[i].selected.amount > 0)
      totalsum += this.state.servicelist[i].selected.amount;
    this.setState({
      loading: false,
      loader: false,
      total_amt: Math.round(totalsum),
      servicelist: [
  ...
  onRowchange(e, val, k) {
    ...
    let sum = 0;
    if (isNaN(this.state.servicelist[k].selected["qty"]))
      this.state.servicelist[k].selected["qty"] = 0;
    let servicelist = this.state.servicelist.map((val, i) => {
      if (
        parseInt(val.selected["qty"]) > 0 &&
        parseInt(val.selected["rate"]) > 0 &&
        (val.selected["servicetype"] == 1 || allocate_id != 2)
      ) {
        val.selected["amount"] = Math.round(
          parseInt(val.selected["qty"]) * parseInt(val.selected["rate"])
        );
        ...
        if (
          val.selected["qty"] == "" ||
          val.selected["qty"] == 0 ||
          val.selected["qty"] == undefined
        )
          val.selected["qty"] = parseInt(1);
        if (diffDays > 0) {
          //console.log("diffDays 290"+diffDays);
          diffDays = Number(diffDays) + 1;
          let totalSerivcePro = 0;
          let daysInMonth = 0;
          let temp = 0;
          for (let mon = date1.getMonth(); mon <= date2.getMonth(); mon++) {
            daysInMonth = new Date(date1.getYear(), mon + 1, 0).getDate();
            if (date1.getMonth() != mon || date2.getMonth() != mon)
              diffDays = daysInMonth;
            if (date1.getMonth() == mon && date2.getMonth() != mon)
              diffDays = daysInMonth - date1.getDate();
            if (date2.getMonth() == mon && date1.getMonth() != date2.getMonth())
              diffDays = date2.getDate();
            ...
            if (temp == 0 && date1.getMonth() != date2.getMonth())
              diffDays = Number(diffDays) + 1;
            temp = 1;
            diffDays = Number(diffDays);
            ...
            totalSerivcePro +=
              parseFloat((val.selected["rate"] / daysInMonth) * diffDays) * 1;
            console.log(
              "Month=" + mon,
              "total daysInMonth=" + daysInMonth,
              "diffDays=" + diffDays,
              "totalSerivcePro=" + totalSerivcePro
            );
          }
          val.selected["prorate"] = Math.round(totalSerivcePro);
          val.selected["amount"] =
            Math.round(val.selected["prorate"]) * parseInt(val.selected["qty"]);
          val.selected["amount"] = Math.round(
            parseInt(val.selected["prorate"]) * parseInt(val.selected["qty"])
          );
          sum = parseFloat(
            parseFloat(sum) + parseFloat(val.selected["amount"])
          );
        } else {
          alert(
            "End Date should be greater than start date for " +
              val.selected["servicename"]
          );
  ...
  handleChange(date) {
    this.state.servicelist[index].selected[cols] = moment(
      new Date(date)
    ).format("YYYY-MM-DD");
    if (
      this.state.servicelist[index].selected["servicetype"] == 0 &&
      this.state.servicelist[index].selected["startDate"] != "" &&
      ...
      if (
        this.state.servicelist[index].selected["qty"] == "" ||
        this.state.servicelist[index].selected["qty"] == 0 ||
        this.state.servicelist[index].selected["qty"] == undefined
      )
        this.state.servicelist[index].selected["qty"] = parseInt(1);
      if (diffDays > 0) {
        let allocate_id = localStorage.getItem("allocate_id");
        if (allocate_id == 2) {
          diffDays = Number(diffDays) + 1;
          var totalSerivcePro = 0;
          var daysInMonth = 0;
          let temp = 0;
          for (var mon = date1.getMonth(); mon <= date2.getMonth(); mon++) {
            ...
            totalSerivcePro +=
              parseFloat(
                (this.state.servicelist[index].selected["rate"] / daysInMonth) *
                  diffDays
              ) * this.state.servicelist[index].selected["qty"];
            console.log(
              "Month=" + mon,
          ...
          this.state.servicelist[index].selected["prorate"] = Math.round(
            totalSerivcePro
          );
          this.state.servicelist[index].selected["amount"] =
            Math.round(this.state.servicelist[index].selected["prorate"]) *
            parseInt(this.state.servicelist[index].selected["qty"]);
        }
      } else {
        alert(
          "End Date should be greater than start date for " +
            this.state.servicelist[index].selected["servicename"]
        );
        this.state.servicelist[index].selected["endDate"] = "";
      }
    }
    ...
    this.state.servicelist.map((val, i) => {
      ...
      if (parseFloat(this.state.servicelist[i].selected["amount"]) > 0)
        totalSum = parseFloat(
          parseFloat(totalSum) +
            parseFloat(this.state.servicelist[i].selected["amount"])
        );
    });
    this.setState({
      servicelist: this.state.servicelist,
  ...
  toggleCalendar(e, col, i) {
    ...
    let pickdate =
      ...
      this.state.servicelist[index].selected[cols]
        ? moment(this.state.servicelist[index].selected[cols])
        : moment();
        ...
        <div className="transition-item detail-page">
          <div className="main-content">
            <div className="col s12 portlet-title">
              <div className="caption">{title} Service Alloaction</div>
            </div>
            <div className="row">
              <div className="portlet_title  col s3">
                {heading}
            ...
            <div className="row">
              <div className="col s3">Company info : {company_name}</div>
              <div className="col s1">Billing Date</div>
                ...
                <div className="col s12">
                  <table className="striped ">
                    <thead className="fixed-header">
                      <tr>
                        <th width="4%">Sno</th>
                        <th className="leftAlignText" width="23%">Service Name</th>
                        <th className="leftAlignText">Category</th>
                        <th className="leftAlignText">From Date</th>
                        <th className="leftAlignText">To Date</th>
                        <th className="leftAlignText">Rate</th>
                        <th className="leftAlignText">Qty</th>
                        <th className="leftAlignText">Total Amount</th>
                      </tr>
                    </thead>
                      ...
                      {servicelist.map((val, i) => {
                        return (
                          <tr key={i}>
                            ...
                            <td className="input-field " width="23%">
                              <Autocomplete
                            ...
                            <td className="input-field">
                              <input
                                type="text"
                                value={val.selected.service_category}
                            ...
                            <td className="input-field">
                              <input
                                ...
                                readOnly={true}
                                onClick={e => {
                                  this.toggleCalendar(e, "startDate", i);
                                }}
                                value={val.selected.startDate}
                            ...
                            <td className="input-field">
                              <input
                                ...
                                readOnly={true}
                                onClick={e => {
                                  this.toggleCalendar(e, "endDate", i);
                                }}
                                value={val.selected.endDate}
                            ...
                            <td className="input-field">
                              <input
                                type="text"
                                onChange={e => {
                                  this.onRowchange(e, "qty", i);
                                }}
                                value={val.selected.qty}
                                placeholder="Qty"
                              />
                            </td>
                            <td className="input-field rightAlignText" >
                              <input
                                type="text"
                                value={val.selected.amount}
                    ...
                    <tbody>
                      {bill_data.map((val, i) => {
                        ...
                        if (val.gst_applicable)
                          totalcgst_amt =
                            Math.round(parseInt(val.amount) * 0.09) +
                            totalcgst_amt;
                        if (val.gst_applicable)
                          totalsgst_amt =
                            Math.round(parseInt(val.amount) * 0.09) +
                        ...
                        return (
                          <tr key={i}>
                            ...
                            <td className="input-field" width="23%">{val.servicename}</td>
