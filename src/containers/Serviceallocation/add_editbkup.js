...
/*    if(localStorage.getItem("allocate_id")==2){ 
   ...
   onChangetext( e ,i) {
    ...
    this.setState({loading:true,loader:true,servicelist:this.state.servicelist});
      ...
      xhr.get('/service',{params:{search_term:e.target.value,limit:10,page:1,allocate_id:allocate_id}},{requestId:requestId}).then((res)=>{
  ...
  onSelectedtext(i,val,list) { 
    ...
    let isvalid = Object.keys(slist).some(x => slist[x].value==val);
     if(!isvalid){
          ...
          this.state.servicelist[i].selected.startDate=(this.state.servicelist[i].selected["servicetype"]==1) ? today :'';
          this.state.servicelist[i].selected.endDate=(this.state.servicelist[i].selected["servicetype"]==1) ? today :'';
          if(this.state.servicelist[i].selected.amount>0)totalsum += this.state.servicelist[i].selected.amount;
         console.log(this.state.total_amt,this.state.servicelist[i].selected.amount,this.state.total_amt,totalsum);
  ...
  onRowchange(e,val,k) { 
    ...
    this.state.servicelist.map((val,i) => {
      ...
      if(parseInt(this.state.servicelist[i].selected["qty"]) > 0 && parseInt(this.state.servicelist[i].selected["rate"]) > 0 && this.state.servicelist[i].selected["servicetype"]==1){
         this.state.servicelist[i].selected["amount"]=Math.round(parseInt(this.state.servicelist[i].selected["qty"])*parseInt(this.state.servicelist[i].selected["rate"]));
          sum=parseFloat(parseFloat(sum)+parseFloat(this.state.servicelist[i].selected["amount"]));           
      }
      else
      {
        if(parseInt(this.state.servicelist[i].selected["prorate"])>0)
        {

         this.state.servicelist[i].selected["amount"]=Math.round(parseInt(this.state.servicelist[i].selected["prorate"])*parseInt(this.state.servicelist[i].selected["qty"]));
            sum=parseFloat(parseFloat(sum)+parseFloat(this.state.servicelist[i].selected["amount"]));           
        }
          
      }
     });
    console.log("onRowchange",this.state.total_amt);

    this.setState({servicelist:this.state.servicelist,total_amt:Math.round(sum)});
  }
...
handleChange (date) {
  ...
  if(this.state.servicelist[index].selected["servicetype"]==0 && this.state.servicelist[index].selected["startDate"] !="" && this.state.servicelist[index].selected["endDate"] !="")
  {
    ...
    if(diffDays>0){      
      var totalSerivcePro=0;var daysInMonth=0;
...
toggleCalendar (e,col,i) {
  e && e.preventDefault()
    ...
    //console.log(e,i,col);
    //console.log(i,col,e);
  //this.state.servicelist[index].selected[cols]=moment(new Date(e.target.value)).format("YYYY-MM-DD");
  //this.state.servicelist[index].selected[cols]=e.target.value;
  ...
  render() {
    ...
    let heading=(allocate_id==2)?'Volume sales':(allocate_id==2)?"Recurring":"One Time";
    let totalcgst_amt=0;
    let totalsgst_amt=0;
