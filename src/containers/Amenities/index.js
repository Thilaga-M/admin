...
import { fetchAmenities,deleteAmenities } from '../../actions/amenitiesActions';
...
class Amenities extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchAmenities(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
    let role=permissionCheck("amenities");
      if(!role)
        return <Nopermission/>
            ...
            <div className="transition-item list-page">
                <div className="main-content">
                  <div className="col s12 portlet-title"> 
                    <div className="caption">AMENITIES
                    </div>
                  </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-amenities":""}/>
                ...
                <table className="striped">
                  ...
                  <tbody>
                  {
                    data.data && data.data.map((val,i)=>{
                      ...
                      return(
                        <tr key={i}>
                            <td>{val.amenities_values}</td> 
                            <td>
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/general/edit-amenities/${val.am_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.am_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
