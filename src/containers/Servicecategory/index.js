...
import { fetchServicecategory,deleteServicecategory } from '../../actions/servicecategoryActions';
...
class Servicecategory extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchServicecategory(self.state.params).then(()=>{
  ...
  delete(id){
    ...
    if(del){
      ...
      deleteServicecategory(id).then((res)=>{
  ...
  PagenationChange(page) {
   ...
   render() {
    let role=permissionCheck("servicecategory");
      if(!role)
        return <Nopermission/>
            ...
            <div className="transition-item list-page">
                <div className="main-content">
                  <div className="col s12 portlet-title"> 
                    <div className="caption">SERVICE CATEGORY
                    </div>
                  </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-servicecategory":""}/>
                { action.indexOf("View")!==-1 &&
                <table className="striped">
                  <thead>
                    <tr>
                      <th className="leftAlignText">Service Category</th>
                      <th>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                  {
                    data.data && data.data.map((val,i)=>{
                      ...
                      return(
                        <tr key={i}>
                            <td className="leftAlignText">{val.service_category}</td>
                            <td>
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/general/edit-servicecategory/${val.sc_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.sc_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchServicecategory },dispatch);
}
