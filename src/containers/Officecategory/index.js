...
import { fetchCategory,deleteCategory } from '../../actions/officecategoryActions';
...
class Officecategory extends React.Component {
  constructor(props) {
    super(props);
    this.state={
        params:{
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchCategory(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
   let role=permissionCheck("officecategory");
      if(!role)
        return <Nopermission/>
             ...
             <div className="transition-item list-page">
                <div className="main-content">
                  <div className="col s12 portlet-title"> 
                    <div className="caption">OFFICE CATEGORY 
                    </div>
                  </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-category":""}/>
                  ...
                  <table className="striped">
                    ...
                    <tbody>
                    {
                      data.data && data.data.map((val,i)=>{
                        ...
                        return(
                          <tr key={i}>
                              <td>{val.officecategory}</td> 
                              <td>
                                { action.indexOf("Edit")!==-1 &&
                                  <Link to={`settings/general/edit-category/${val.ocm_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                                }&nbsp;&nbsp;
                                {action.indexOf("Delete")!==-1 &&
                                  <span onClick={(e)=>{this.delete(val.ocm_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                                }
                              </td>
                          </tr>
                          )
