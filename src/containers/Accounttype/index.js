...
import { fetchAccounttype,deleteAccounttype } from '../../actions/accounttypeActions';
...
class Accounttype extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchAccounttype(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
    let role=permissionCheck("accounttype");
      if(!role)
        return <Nopermission/>
          ...
          <div className="portlet">
            <div className="transition-item list-page">
                ...
                <div className="col s12 portlet-title"> 
                    <div className="caption">ACCOUNT TYPE
                    </div>
                </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/accounts/add-accounttype":""}/>
                      ...
                      return(
                        <tr key={i}>
                            ...
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/accounts/edit-accounttype/${val.acctypeid}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.acctypeid)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchAccounttype },dispatch);
}
