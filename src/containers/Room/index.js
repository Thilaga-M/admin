...
import { fetchRoom,deleteRoom } from '../../actions/RoomActions';
...
class RoomMaster extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchRoom(self.state.params).then(()=>{
            ...
            <div className="transition-item list-page">
                <div className="main-content">
                  <div className="col s12 portlet-title"> 
                    <div className="caption">ROOM
                    </div>
                  </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-room":""}/>
                ...
                <table className="striped">
                  <thead>
                    <tr>           
                      ...
                      <th>Room Name</th>
                      <th className="leftAlignText">Room Description</th>  
                      <th>Room Capacity</th> 
                      <th>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                  {
                    data.data && data.data.map((val,i)=>{
                      ...
                      return(
                        <tr key={i}>
                            <td className="leftAlignText">{val.floorname}</td>
                            <td>{val.room_name}</td>
                            <td className="leftAlignText">{val.description}</td>
                            <td>{val.capacity}</td>                               
                            <td>
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/general/edit-room/${val.room_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.room_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
