...
import { fetchTemplateCategory,deleteTemplateCategory } from '../../actions/templatecategoryActions';
...
class Templatecategory extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchTemplateCategory(self.state.params).then(()=>{
  ...
  delete(id){
    ...
    if(del){
      ...
      deleteTemplateCategory(id).then((res)=>{
            ...
            <div className="transition-item list-page">
                <div className="main-content">
                  <div className="col s12 portlet-title"> 
                      <div className="caption">TEMPLATE CATEGORY
                      </div>
                    </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-templatecategory":""}/>
                      ...
                      return(
                        <tr key={i}>
                            ...
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/general/edit-templatecategory/${val.id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchTemplateCategory },dispatch);
}
