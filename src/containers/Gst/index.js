...
import { fetchGst,deleteGst } from '../../actions/gstActions';
...
class Gst extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchGst(self.state.params).then(()=>{
          ...
          <div className="portlet">
            <div className="transition-item list-page">
                ...
                <div className="col s12 portlet-title"> 
                      <div className="caption">GST
                      </div>
                    </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1 && data.data.length>3)?"settings/accounts/add-gst":""}/>
                      ...
                      return(
                        <tr key={i}>
                            ...
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/accounts/edit-gst/${val.gst_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchGst },dispatch);
}
