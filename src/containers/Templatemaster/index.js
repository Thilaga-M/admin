...
import { fetchTemplateMaster,deleteTemplateMaster } from '../../actions/templateMasterActions';
...
class Templatemaster extends React.Component {
  constructor(props) {
    super(props);
  this.state={
        params:{
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchTemplateMaster(self.state.params).then(()=>{
  ...
  delete(id){
    ...
    if(del){
      ...
      deleteTemplateMaster(id).then((res)=>{
            ...
            <div className="transition-item list-page">
                <div className="main-content">
                  <div className="col s12 portlet-title"> 
                      <div className="caption">TEMPLATE MASTER
                      </div>
                    </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-templatemaster":""}/>
                      ...
                      return(
                        <tr key={i}>
                            ...
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/general/edit-templatemaster/${val.template_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.template_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchTemplateMaster },dispatch);
}
