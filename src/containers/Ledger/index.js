...
import { fetchLedger,deleteLedger } from '../../actions/ledgerActions';
...
class Ledger extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchLedger(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
    let role=permissionCheck("ledger");
      if(!role)
        return <Nopermission/>
          ...
          <div className="portlet">
            <div className="transition-item list-page">
                ...
                <div className="col s12 portlet-title"> 
                      <div className="caption">LEDGER
                      </div>
                    </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/accounts/add-ledger":""}/>
                      ...
                      return(
                        <tr key={i}>
                            ...
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/accounts/edit-ledger/${val.ledgerid}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.ledgerid)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchLedger },dispatch);
}
