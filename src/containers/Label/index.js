...
import { fetchLabel,deleteLabel } from '../../actions/labelActions';
...
class Label extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchLabel(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
    let role=permissionCheck("label");
      if(!role)
        return <Nopermission/>
       ...
       return (
          <div className="portlet">
            <div className="transition-item list-page">
                ...
                <Filter  filterSearch={this.filterSearch.bind(this)}  addbtn={(action.indexOf("New")!==-1)?"settings/general/add-label":""}/>
                    ...
                    data.data && data.data.map((val,i)=>{
                      ...
                      return(
                        <tr key={i}>
                            ...
                            <td>{val.label_value}</td>
                            <td>
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/general/edit-label/${val.lbl_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.lbl_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchLabel },dispatch);
}

const LabelContainer = connect(mapStateToProps,
