...
import { fetchNotification,deleteNotification } from '../../actions/notificationActions';
...
class Notification extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchNotification(self.state.params).then(()=>{
            ...
            <div className="transition-item list-page">
                <div className="main-content">
                  <div className="col s12 portlet-title"> 
                      <div className="caption">NOTIFICATION
                      </div>
                    </div>
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"customers/notification/add-notification":""}/>
                { action.indexOf("View")!==-1 &&
                <table className="striped">
                  <thead>
                    <tr>                      
                      <th className="leftAlignText">Notification Name</th>
                      <th className="leftAlignText">Notification Description</th> 
                      <th>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                  {
                    data.data && data.data.map((val,i)=>{
                      ...
                      return(
                        <tr key={i}>
                            <td className="leftAlignText">{val.notification_name}</td> 
                            <td className="leftAlignText">{val.notification_description}</td>                            
                            <td>
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`customers/notification/edit-notification/${val.notification_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.notification_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchNotification },dispatch);
}
