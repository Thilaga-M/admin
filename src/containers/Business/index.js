...
class Business extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchBusiness(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
   let role=permissionCheck("business");
   if(!role)
        return <Nopermission/>
    ...
    let total=(data)?data.total:0;
       ...
       return (
             <div className="transition-item list-page">
                ...
                <Filter  filterSearch={this.filterSearch.bind(this)}  addbtn={(action.indexOf("New")!==-1)?"settings/add-business":""}/>
                              ...
                              <table className="striped">
                                      ...
                                      <tr key={i}>
                                          ...
                                          <td>
                                             { action.indexOf("Edit")!==-1 &&
                                              <Link to={`settings/edit-business/${val.business_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                                              }&nbsp;&nbsp;
                                            {action.indexOf("Delete")!==-1 &&
                                              <span onClick={(e)=>{this.delete(val.business_id)}} ><i className="material-icons dp48">delete</i></span>
                                              }
                                          </td>
                                      </tr>
                                      )
                                ...
                                }
                                {
                                  (data.data && data.data.length===0 )&&
                                    <tr>  
                                        <td colSpan="8" className="center red-text">No Record Found...!</td> 
                                    </tr> 
                                }
...
const BusinessContainer = connect(mapStateToProps,
