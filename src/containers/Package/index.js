...
import { fetchPackage,deletePackage } from '../../actions/packageActions';
...
class Package extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchPackage(self.state.params).then(()=>{
    ...
    let action=(role.permission)?role.permission.split(","):[];
       return (
            <div className="transition-item list-page">
                ...
                <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/add-package":""}/>
                      ...
                      return(
                        <tr key={i}>
                            ...
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/edit-package/${val.pkm_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.pkm_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const PackageContainer = connect(mapStateToProps,
