...
import { fetchLanguage,deleteLanguage } from '../../actions/languageActions';
...
class Language extends React.Component {
  ...
  fetchData(){
    let self = this;
    setTimeout(()=>{
       self.props.fetchLanguage(self.state.params).then(()=>{
  ...
  PagenationChange(page) {
   ...
   render() {
    let role=permissionCheck("language");
      if(!role)
        return <Nopermission/>
          ...
          <div className="portlet">
            <div className="transition-item list-page">
                <div className="main-content">
                 ...
                 <Filter  filterSearch={this.filterSearch.bind(this)} addbtn={(action.indexOf("New")!==-1)?"settings/general/add-language":""}/>
                { action.indexOf("View")!==-1 &&
                <table className="striped">
                  <thead>
                    <tr>
                      <th className="leftAlignText">Language Name</th>
                      <th>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                  {
                    data.data && data.data.map((val,i)=>{
                      ...
                      return(
                        <tr key={i}>
                            <td className="leftAlignText">{val.language_name}</td>
                            <td>
                            { action.indexOf("Edit")!==-1 &&
                              <Link to={`settings/general/edit-language/${val.language_id}`}><i className="material-icons dp48 invalid">edit</i></Link>
                            }&nbsp;&nbsp;
                            {action.indexOf("Delete")!==-1 &&
                              <span onClick={(e)=>{this.delete(val.language_id)}} ><i className="material-icons dp48 invalid">delete</i></span>
                            }
                            </td>
                        </tr>
                        )
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({ fetchLanguage },dispatch);
}

const LangaugeContainer = connect(mapStateToProps,
  mapDispatchToProps, null, { withRef: true }
)(Language)

export default LangaugeContainer;