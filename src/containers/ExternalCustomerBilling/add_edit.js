...
import { addServiceallocation , update_fetch_status, viewServiceallocation, servicemasterlist,viewBill,confirmBill,getExternalCustomers} from '../../actions/externalbillingActions';
...
function validate(cust_hid)
{
  return {
    cust_hid:cust_hid.length===0
  }
}
var index='',cols='';
class AddEdit extends React.Component {
  constructor(props) {
   ...
   if(localStorage.getItem("allocate_id")==2){ 
    ...
    componentWillMount(){
      localStorage.setItem("allocate_id",2);
     ...
     if(sa_hid){
      ...
      getExternalCustomers().then((res)=>{
        this.setState({preloader:true}); 
        if(res.data.status===200){
          this.setState({externalCustomerList:res.data.result.data,preloader:false}); 
        }
      })
    }
    ...
    this.state.servicelist.map((val,i) => {
      ...
      if(parseInt(this.state.servicelist[i].selected["amount"]) == 0 ) {
        ...
        let {customer_name,company_name,address,customer_type,discount,agreement_start_date,agreement_end_date,office_space_ids,allocate_office_price,annexure,allocate_discount_price,status,servicelist,total_amt,billingDate,cust_hid} =this.state;
...
toggleCalendar (e,col,i) {
  ...
  render() {
    let {bill_data,bill_confirm,customer_name,company_name,address,customer_type,discount,agreement_start_date,agreement_end_date,office_space_ids,allocate_office_price,annexure,status,servicelist,allocate_discount_price,error_status,title,preloader,value,areas,area,qty,amount,total_amt,externalCustomerList,cust_hid} = this.state;
                  ...
                  <div className="transition-item detail-page">
                   <div className="main-content">
                      <div className="col s12 portlet-title"> 
                        <div className="caption">EXTERNAL BILLING
                        </div>
                      </div>
                        ...
                        <div className="row">
                            ...
                            <select className={`browser-default ${(errors.cust_hid && error_status)?'invalid':''}`} value={cust_hid}  onChange={(e)=>{this.eventHandle(e,"cust_hid")}} >
                                  <option value=''>Select Company Name</option>
                                  {
                                    externalCustomerList && externalCustomerList.map((val,i)=>{
                          ...
                          <div className="input-field col s2">Billing Date:</div>
                            <div className="input-field col s2">  
                            ...
                            <div className="col s12">
                                  <table className="striped ">
                                    <thead className="fixed-header">
                                      <tr>
                                        <th width="4%">Sno</th>
                                        <th className="input-field leftAlignText" width="23%">Service Name</th>
                                        <th>Category</th> 
                                        <th>From Date</th>
                                        <th>To Date</th>
                                        <th>Rate</th>
...

                                    {
                                       servicelist.map((val,i)=>{
                                            ...
                                            return(
                                              ...
                                              <td className="input-field leftAlignText" width="23%">
                        ...
                        { bill_confirm===0 &&
                          <div className="row btn-margin"> 
                           <div className="card-footer right">
                            &nbsp;&nbsp;
                            <button type="button"  onClick={this.openModal}  className="btn btn-sm btn-primary">{(allocate_id)?"Save & Bill":"Save"}</button>
                            &nbsp;&nbsp;
                          ...
                          <Modal
                            ...
                            <div>Are you sure want to save the Bill??</div>
                           
                              <input style={{borderBottom: 'none'}}/>
                              <button className="btn btn-sm btn-primary" style={{margin: '1%'}} onClick={this.onsubmit.bind(this)}>Save</button>
                              <button className="btn btn-sm btn-default"  style={{margin: '1%'}} onClick={this.closeModal.bind(this)}>close</button>
                        ...
                        { bill_confirm===1 &&
                          <div className="row"> 
                           <div className="card-footer right"> 
                            <button type="button" onClick={this.onsubmitnew.bind(this)} className="btn btn-sm btn-primary">Confirm Bill</button>
                            &nbsp;&nbsp;
                            <Link to="customers/serviceallocation" className="btn btn-sm btn-default">Cancel</Link>
                          </div>
                          </div> 
