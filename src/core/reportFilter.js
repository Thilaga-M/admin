...
class ReportFilter extends Component {
  ...
  applyFilter() {
    this.props.filterSearch({
      ...
      refundableDeposit: this.state.refundableDeposit
    });
  }
  handleclick(e){
  ...
  render() {
    ...
    return (
      <div className="filter">
        <div className="filterSearch" style={{ marginBottom: "0%" }}>
          ...
          <div className="input-field col s2" style={{ marginTop: "2px",margin:'5px'}}>
            <select
              value={periodFilter}
              onChange={e => {
                this.eventHandle(e, "periodFilter");
              }}
            >
              <option value="">Select Period Type</option>
              <option value="1">Bill Date</option>
              <option value="2">Bill Period</option>
            </select>
          </div>

          {this.props.displayType != "false" && (
            <div className="input-field col s2" style={{ marginTop: "2px" }}>
              <select  value={displayFilter}
                onChange={e => {
                  this.eventHandle(e, "displayFilter");
                }}>
                <option value="">Select Display Type</option>
                <option value="Split">Customer Wise</option>
                <option value="Consolidated">Service Category Wise</option>
              </select>
            </div>
          )}
          ...
          <div className="input-field col s2" style={{ marginTop: "2px" ,margin:'5px'}}>
            <select
              value={service_name}
              onChange={e => {
                this.eventHandle(e, "service_name");
              }}
            >
              <option value="">Select Service Name</option>
              {this.state.serviceItems &&
                this.state.serviceItems.map((val, i) => {
                  return (
                    <option title={i} key={i} value={val.service_name}>
                      {val.servicename}
                    </option>
                  );
                })}
        ...
        <div className="row">
          {this.props.isIncomeTypeFilter && (
            <div className="input-field col s2" style={{ marginTop: "2px" ,margin:'5px'}}>
              <select
                value={incomeType}
                onChange={e => {
                  this.eventHandle(e, "incomeType");
                }}
              >
          ...
          {this.props.isRefundableFilter && (
            <div className="input-field col s2" style={{ marginTop: "2px" ,margin:'5px'}}>
              <select
                value={refundableDeposit}
                onChange={e => {
                  this.eventHandle(e, "refundableDeposit");
                }}
              >
                <option value="">Include refundable deposit?</option>
          ...
          <div className="col s10" style={{margin:'5px'}}>
            <button
              style={{ float: "left" }}
              type="button"
